...
module.exports = steemAPI => {
  ...
  function processOrders(open_orders, assetPrecision) {
    const sbdOrders = !open_orders
      ...
      : open_orders.reduce((o, order) => {
          if (order.sell_price.base.indexOf("GBD") !== -1) {
            o += order.for_sale;
          }
          return o;
        }, 0) / assetPrecision;
    ...
    const steemOrders = !open_orders
      ? 0
      : open_orders.reduce((o, order) => {
          if (order.sell_price.base.indexOf("GNEX") !== -1) {
            o += order.for_sale;
          }
          return o;
        }, 0) / assetPrecision;
  ...
  function calculateSaving(savings_withdraws) {
    ...
    savings_withdraws.forEach(withdraw => {
      ...
      if (asset === "GNEX") savings_pending += parseFloat(amount);
      else {
        if (asset === "GBD") savings_sbd_pending += parseFloat(amount);
      }
    });
  ...
  function pricePerSteem(feed_price) {
    ...
    if (/ GBD$/.test(base) && / GNEX$/.test(quote)) {
    ...
    return Promise.all(promises).then(() => {
      ...
      (account.other_history || []).reduce((out, item) => {
        ...
        const amount = parseFloat(
          get(item, [1, "op", 1, "amount"]).replace(" GBD", "")
        );
